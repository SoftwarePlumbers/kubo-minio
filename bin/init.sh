...
function init_function() {
  if [ -e "$IPFS_PATH/config" ]; then
    ...
    if [ ! -z "$IPFS_MINIO_SERVICE" ]; then
      ...
      DATASTORE_SPEC_FILE='{"mounts":[{"bucket":"'$IPFS_MINIO_BUCKET'","mountpoint":"/blocks","region":"us-east-1","rootDirectory":"blocks"},{"mountpoint":"/","path":"datastore","type":"levelds"}],"type":"mount"}'
